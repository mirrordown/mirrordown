...
processor.processSync('!![A wild rice field](./wild-rice.jpg "Wild rice")');
```

The plugin wraps each opted-in image in an inline trigger button and appends the
...
- **Name a lightbox with `{#id}`.** With
  [remark-attrs](https://www.npmjs.com/package/@mirrordown/remd-attrs) (or any
  compatible attrs plugin), `!![Alt](photo.jpg){#hero}` uses `hero` as the
  dialog `id` instead of the hashed default. Run the attrs plugin before
  `rehypeLightbox`.
