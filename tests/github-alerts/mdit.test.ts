...
test.each(mainCases)("github-alerts (markdown-it): $name", ({ input }) => {
  expect(normalizeHtml(md.render(input))).toMatchSnapshot();
});

test("github-alerts (markdown-it): no icons", () => {
  expect(normalizeHtml(mdNoIcons.render(noIconsCase.input))).toMatchSnapshot();
});
